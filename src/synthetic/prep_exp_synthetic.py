...
from src.operations.preprocess import ProtectedAttr
from src.readers import file_reader
from src.utils import parse_args, cross_tab_percent, get_map_oc_str, get_map_sc_str, read_parameters, \
    get_data_loaders, filter_indexes, generic_synthetic_data
...
if __name__ == "__main__":
    ...
    data, set_name_c = generic_synthetic_data(params['synthetic_mode'], save_path=experiment_dir)

    loaders = get_data_loaders(df=data, set_name=set_name_c, target=params['target'])

    model = Predictor(dimensions=params["model_dimensions"], lr=params["model_lr"], seed=987654321)
    ...
    log_data_info(df=data, set_name=set_name_column, pa_series=pa_column, target=params['target'],
                  pa_name=attr_name)
